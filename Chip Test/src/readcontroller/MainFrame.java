...
public class MainFrame extends JFrame {
	...
	final JButton button1, button2, button0, button100, buttonN, buttonCalc,
			button0D0, button100D0, buttonND0, buttonCalcD0, buttonCancelD1,
			buttonCancelD0;
		...
		sh.sc = new SerialCallback() {
			...
			public void onProcessed(byte[] buffer, int code) {
				...
				case 'A':
					...
					int yCount = Integer.valueOf(response.substring(0,
							response.length()));
				...
				case 'B':
					...
					// response.indexOf(' ')));
					yCount = Integer.valueOf(response.substring(0,
							response.length()));
	...
	private void initTestingPanel() {
		...
		testLabel.setBounds(100, 100, 600, 400);
	...
	private void initCalibrationPdanel() {

		graphPanel = new JPanel();
		graphPanel.setLayout(null);
		label5 = new JTextArea(
				"Чтобы получить смещение нуля, нажмите Value 0mA.\nЧтобы получить значение с АЦП равным 100мА нажмите Value 100mA.\nЧтобы получить текущее значение тока, нажмите Value NmA. Чтобы рассчитать \nзначение тока на текущей линии нажмите Calc\nПервый график соответствует значениям с линии D0, второй - с D1.");
		// label5.setSize(400, 500);
		label5.setBounds(10, 10, 570, 100);
		...
		button5.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				graphBuilder.resetPointArray();
				graphBuilder.resetTimeStarted();
		...
		buttonCalcD0.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent a) {
				...
				float textFieldND0_ = Float.valueOf(textFieldND0.getText()
						.replaceAll(" \r\n", ""));
				float textField0D0_ = Float.valueOf(textField0D0.getText()
						.replaceAll(" \r\n", ""));
				float textField100D0_ = Float.valueOf(textField100D0.getText()
						.replaceAll(" \r\n", ""));

				textFieldCalcD0.setText(Float
						.toString((textFieldND0_ - textField0D0_) * 100
								/ textField100D0_));
		...
		buttonCalc.addActionListener(new ActionListener() {
			...
			@Override
			public void actionPerformed(ActionEvent a) {
				float textFieldN_ = Float.valueOf(textFieldN.getText()
						.replaceAll(" \r\n", ""));
				float textField0_ = Float.valueOf(textField0.getText()
						.replaceAll(" \r\n", ""));
				float textField100_ = Float.valueOf(textField100.getText()
						.replaceAll(" \r\n", ""));

				textFieldCalc.setText(Float
						.toString((textFieldN_ - textField0_) * 100
								/ textField100_));
		...
		tabs.addChangeListener(new ChangeListener() {
			...
			public void stateChanged(ChangeEvent a) {
				if (tabs.getSelectedIndex() == 1) {
					...
					graphBuilder = new TimedGraphBuilder(700, 100, 570, 300,
							graphPanel, mainFrame);
					graphBuilder.setMaxXAxisRange(7.0f);
					graphBuilder.setMaxYAxisRange(2048);
					graphBuilder.setXSkipStep(2);
					graphBuilder.init(true);

					graphBuilderD0 = new TimedGraphBuilder(700, 420, 570, 300,
							graphPanel, mainFrame);
