...
//	}
	...
	protected void drawYAxis(){
		...
		for (int i = 1; i < yAxisUnitDiv; i++){
			//yAxisDivLengthInPrc - длина чёрточки в процентах от общей ширины
	...
	protected void drawXAxis(){
		...
		for (int i = 1; i < xAxisUnitDiv; i++){
			...
			if (i % xSkipStep == 0){
				//xMinRangeValue - значение самого левого края 
				...
				//(y1_ + xAxisValueOffset * height) - насколько нужно сдвинуть вниз
				g.drawString(rr, x1_, (int) (y1_ + xAxisValueOffset * height));
	...
	public void drawFromTo(int from, int to){
		...
		for (int i = from; i < to - 1; i++){
			g.drawLine(x0 + (int)(pointArray.get(i).x * width / xMaxRangeValue), y0 + (int)pointArray.get(i).y * height / , 
					x0 + (int)(pointArray.get(i+1).x * width / xMaxRangeValue), y0 + (int)pointArray.get(i+1).y);
		}*/
